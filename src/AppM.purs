...
import Conduit.Foreign.History (setUrl)
...
newtype AppM v a = AppM (ReaderT Env (Widget v) a)

-- | Fantastic! We can now implement our custom application monad with our `AppM` type. However,
-- | as described in detail in the `Main` module, Concur has no idea how to run an application in
...
runAppM :: forall v. Env -> AppM v ~> Widget v
...
derive newtype instance functorAppM :: Functor (AppM v)
derive newtype instance applyAppM :: Apply (AppM v)
derive newtype instance applicativeAppM :: Applicative (AppM v)
derive newtype instance bindAppM :: Bind (AppM v)
derive newtype instance monadAppM :: Monad (AppM v)
derive newtype instance monadEffectAppM :: Monoid v => MonadEffect (AppM v)
derive newtype instance monadAffAppM :: Monoid v => MonadAff (AppM v)
...
-- | ```
instance monadAskAppM :: TypeEquals e Env => MonadAsk e (AppM v) where
  ask = AppM $ asks from

-- | The way Concur incorporates other monads is with a class called `ShiftMap`
-- | Once we have a way to map natural transformations for widgets into those for another monad,
-- |  we can then use values for that monad anywhere instead of Widgets.
-- | We can implement `ShiftMap` for `AppM`, so that all widget operations are available to us
instance shiftMapAppM :: ShiftMap (Widget v) (AppM v) where
  shiftMap f (AppM rw) = AppM (shiftMap f rw)
...
instance nowAppM :: Monoid v => Now (AppM v) where
...
instance logMessagesAppM :: Monoid v => LogMessages (AppM v) where
  logMessage log = do
    env <- ask
...
-- | TODO: Implement routing with purescript-concur-react-router?
-- |   Currently, we just use window.history.pushState(_state, _title, url)
-- | Note how our navigation capability uses our routing data type rather than let you set any
-- | arbitrary url. Logging out is a little more involved, because we need to clean up things like
-- | the auth token. Navigating home will take care of emptying the reference to the current user.
instance navigateAppM :: Monoid v => Navigate (AppM v) where
  navigate =
    liftEffect <<< setUrl <<< print Route.routeCodec
...
instance manageUserAppM :: Monoid v => ManageUser (AppM v) where
  loginUser =
...
instance manageTagAppM :: Monoid v => ManageTag (AppM v) where
  getAllTags =
    mkRequest { endpoint: Tags, method: Get }
      >>= decode (decodeAt "tags")

-- | Our operations for managing comments
instance manageCommentAppm :: Monoid v => ManageComment (AppM v) where
  getComments slug =
...
instance manageArticleAppM :: Monoid v => ManageArticle (AppM v) where
  getArticle slug =
